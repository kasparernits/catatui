...
const CMDS: Cmd[] = [
  ...
  { title: "Memory", cmd: "free -h || vm_stat 2>/dev/null | head -n 10" },
  { title: "Interfaces", cmd: "ip -brief address || ifconfig" },
  { title: "Routes", cmd: "ip route || netstat -rn" },
  { title: "Listening TCP", cmd: "ss -ltnp || netstat -ltn" },
  { title: "Top procs (mem)", cmd: "ps aux --sort=-%mem | head -n 10" },
  { title: "Dmesg tail", cmd: "dmesg -T | tail -n 40 || dmesg | tail -n 40" },
  {
    title: "Journal (tail)",
    cmd: "journalctl -n 50 --no-pager 2>/dev/null || echo 'journalctl not available'",
  },
  { title: "Who", cmd: "who" },
...
const base = vstack([
  {
    render(a, ctx) {
      ...
      const title = "CATATUI — master/detail (keyboard only)";
